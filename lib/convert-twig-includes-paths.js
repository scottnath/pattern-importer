...
function createNewCategoryPath (options,includePath) {
  ...
  var one = utils.categoryNameConverter(options.convertCategoryTitlesData['categories'], paths[0]);
  newIncludePath.push(one);

  // check paths for four parts - denotes a subcategory
  if(paths.length === 4){
    var two = utils.categoryNameConverter(options.convertCategoryTitlesData.subcategories[one], paths[1]);
