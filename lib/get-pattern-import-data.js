...
module.exports = function getPatternImportData (paths, options) {
  ...
  // determin the pattern's styles destination directory
  patternFiles.patternScriptsPath = path.join(options.scriptsDest,'pattern-scripts',patternFiles.patternCategoryPath);
  ...
  if(patternTemplate){
    ...
    if(options.compilePatternsOnImport){
      ...
      // convert twig includes IF switching category titles in DEST
      // if(options.convertCategoryTitles && options.convertCategoryTitlesDataFile && (options.templateEngine === 'twig')){
