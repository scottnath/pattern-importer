...
clog('options',options);
  ...
  clog('options.convertCategoryTitles',options.convertCategoryTitles);
  ...
  if(options.convertCategoryTitles && options.convertCategoryTitlesDataFile){
    ...
    options.convertCategoryTitlesData = plUtils.convertYamlToObject( convertCategoryTitlesDataFile );
  }
...
function writeCopyFiles (patternFiles) {
  ...
  if((patternFiles.filesToWrite) && Array.isArray(patternFiles.filesToWrite)){
    patternFiles.filesToWrite.forEach(function(file){
      if(file.type === 'data'){
        file.contents = JSON.stringify(file.contents, null, 2);
      }
