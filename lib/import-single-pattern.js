...
var importPattern = function (file, options, compiledPatterns) {
 ...
 *
 * @return {Object} patternFiles
 * @return {Array} patternFiles.filesToWrite   array of file objects
 * @return {String} patternFiles.filesToWrite[file].type   type of file
 * @return {String} patternFiles.filesToWrite[file].dest   relative path to file destination
 * @return {String} patternFiles.filesToWrite[file].contents   contents to be put in the file when we write it
 * @return {Array} patternFiles.filesToCopy   array of file objects
 * @return {String} patternFiles.filesToCopy[file].type   type of file
 * @return {String} patternFiles.filesToCopy[file].src   relative path to file source
 * @return {String} patternFiles.filesToCopy[file].dest   relative path to file destination
 * @return {Object} patternFiles.includedFiles   Contains all css & js files required for this pattern (including included-pattern files)
 * @return {Array} patternFiles.includedFiles.css  relative path to css file's destination
 * @return {Array} patternFiles.includedFiles.js  relative path to js file's destination
 * @return {String} patternFiles.patternCss  relative path for this pattern's main COMPILED css file
 * @return {String} patternFiles.patternScript  relative path for this pattern's main js file

*/
function getPattern (paths, options, compiledPatterns) {
  ...
  if(patternObject.includes){
    ...
    patternObject.includes.forEach(function (include){
      ...
      var includeFile = plUtils.createFile(path.join(paths.folder, include, options.dataFileName), process.cwd(), path.join(paths.folder, include));

      // compile the pattern, return it's included files
      var includePatternFiles = getPattern(plUtils.getFilePaths(includeFile), options, compiledPatterns);
      if(includePatternFiles.patternCss){
        patternFiles.includedFiles.css.push(includePatternFiles.patternCss);
      }
      if(includePatternFiles.patternScript){
        patternFiles.includedFiles.js.push(includePatternFiles.patternScript);
      }
      if((includePatternFiles.filesToWrite) && Array.isArray(includePatternFiles.filesToWrite)){
        includePatternFiles.filesToWrite.forEach(function(file){
          patternFiles.filesToWrite.push(file);
        });
      }
      if((includePatternFiles.filesToCopy) && Array.isArray(includePatternFiles.filesToCopy)){
  ...
  var ymlFileToWrite = {
    ...
    contents: plUtils.convertObjectToYaml(compiledYmlObject)
  }
  ...
  if(!finalPatternCompilerData.data){
 ...
 *
 */
function writePattern (patternFiles) {
